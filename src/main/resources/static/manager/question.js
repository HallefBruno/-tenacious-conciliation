...
function next() {
    ...
    nextdoner.click(function () {
        ...
        $("#href-doner").addClass("active");
        verificaAltenticidadeHtml(nextdoner, "#doner");
    });
    
    nextdoacao.click(function () {
        $("#href-doner").removeClass("active");
        $("#href-doacao").addClass("active");
...
function numeracaoCondominioCasa() {
    ...
    numInicial.on("keyup", function () {
        var options;
        if(Number(numInicial.val().trim() > 0)) {
        ...
        } else if (Number(numInicial.val() < 0)) {
            Swal.fire("Atenção!", "Número precisa ser maio que zero", "warning");
            numInicial.val("");
        } else if(Number(numInicial.val()).toString() ===  "NaN"){
            Swal.fire("Atenção!", "Caracter inválido", "warning");
            numInicial.val("");
        }
    });
}
...
function evetCheck() {
    ...
    $("#checkcn").change(function () {
        ...
        if(prop) {
            ...
            $("#div-somente-casas").html(htmlProfileFinalizado());
            nextmessage.css("display","block");
            qtdCasas = $("input[name='qtdcasas']");
            numInicial = $("input[name='numinicial']");
            ...
            $.ajax({
                ...
                statusCode: {
                    200: function (data, textStatus, jqXHR) {
                        Swal.fire("Atenção!",data.responseText,"success");
                        $("#div-somente-casas").html("");
                        $("#div-somente-casas").append(htmlProfileFinalizado());
                        nextmessage.css("display","block");
                    },
                    304: function (data, textStatus, jqXHR) {
                        Swal.fire("Antenção!","Uma versão foi gerada para esse usuário","warning");
                    },
                    404: function (data, textStatus, jqXHR) {
                        Swal.fire("Atenção!",data.responseText,"warning");
                    },
                    
                    511: function (data, textStatus, jqXHR) {
                        window.console.info(data);
                    }
                },
                
                error: function (data, textStatus, jqXHR) {
                    Swal.fire("Atenção!",data.responseText,"error");
                }
                ...
                "<div class='panel-body'>"+
                    ...
                    "<div class='feed-quantidade'>"+
                        "<div class='form-group col-sm-6'>"+
                            ...
                            "<input type='text' name='qtdcasas' id='qtdcasas' placeholder='Ex: 70' maxlength='3' class='form-control' />"+
                        "</div>"+
                    "</div>"+

                    "<div class='form-group col-sm-6'>"+
                        "<label for='numinicial' class='control-label'>Inicia em que número</label>"+
                        "<input type='text' name='numinicial' id='numinicial' placeholder='Ex: 101' maxlength='4' class='form-control' />"+
                    "</div>"+
                    ...
                    "<div class='feed-codigo-validacao'>"+
                        "<div class='form-group col-sm-6'>"+
                            ...
                            "<input type='number' name='codigo' id='codigo' placeholder='Código que enviamos por e-mail' minlength='4' maxlength='8' class='form-control' />"+
                        "</div>"+
                    "</div>"+
...
function htmlProfileFinalizado() {
    return "<div class='container-fluid' style='height: 450px'><div class='row'><div style='margin-left: 45%; margin-top: 15%;' class='form-group col-sm-6'><span class='round-tabs ' style='color: #3c763d;border: 2px solid #3c763d;'><i class='glyphicon glyphicon-ok'></i></span></div> </div> </div>";
}
